    ...
    if (phone) {
      try {
        ...
        if (response.whatsapp_sid) {
          toast.success('Invoice PDF sent directly to WhatsApp!');
        } else if (response.whatsapp_link) {
          window.open(response.whatsapp_link, '_blank');
          toast.success(response.note || 'WhatsApp opened with invoice details');
        }
      } catch (error: any) {
        toast.error(error.response?.data?.error || 'Failed to share via WhatsApp');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleEmail = useCallback(async () => {
    ...
    if (email) {
      try {
        ...
        toast.success(`Invoice PDF sent successfully to ${email}!`);
      } catch (error: any) {
      ...
      <div className="max-w-2xl w-full fade-in">
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-10">
          <div className="text-center mb-8">
            <div className="w-24 h-24 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl success-icon">
              <svg className="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-3">Invoice Generated!</h1>
            <p className="text-xl text-gray-600 dark:text-gray-300">Your invoice is ready to download</p>
          </div>

          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 rounded-2xl p-6 mb-8 border border-blue-100 dark:border-blue-800">
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-blue-200">
              <span className="subtext font-medium">Invoice Number</span>
              <span className="text-xl font-bold text-blue-600">{invoice.invoice_number}</span>
            </div>
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="subtext">Buyer</span>
                <span className="font-semibold text-gray-900 dark:text-gray-100">{invoice.buyer_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="subtext">Subtotal</span>
                <span className="font-semibold dark:text-gray-200">₹{invoice.subtotal}</span>
              </div>
              <div className="flex justify-between">
                <span className="subtext">GST</span>
                <span className="font-semibold dark:text-gray-200">₹{totalGST}</span>
              </div>
              <div className="flex justify-between text-2xl font-bold border-t border-blue-200 dark:border-blue-800 pt-3">
